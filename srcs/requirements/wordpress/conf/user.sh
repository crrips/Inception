...
if wp user get crrips >/dev/null 2>&1; then
    echo "User crrips exists."
else
   wp user create ${WP_USR} ${WP_EMAIL} --role=subscriber --user_pass=${WP_PWD}
fi
wp user update "apiloian" --user_pass="${WP_PWD}" --skip-email
if wp user get "apiloian" >/dev/null 2>&1; then
